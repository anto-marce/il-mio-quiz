...
def prepara_dati():
    lista = []
    try:
        # 'utf-8-sig' ignora il carattere strano invisibile di Excel
        df = pd.read_csv("quiz.csv", header=None, sep=None, engine='python', encoding='utf-8-sig')
    except Exception as e:
        return f"Errore apertura file: {e}"

    for index, row in df.iterrows():
        testo = str(row[0]).strip()
        if not testo: continue
        
        # Regex migliorata: cerca Numero, Domanda, a), b), c) ignorando i ritorni a capo
        m = re.search(r"(\d+)\s+(.*?)\s+[Aa]\s*[\)\s]\s*(.*?)\s+[Bb]\s*[\)\s]\s*(.*?)\s+[Cc]\s*[\)\s]\s*(.*)", testo, re.DOTALL | re.IGNORECASE)
    ...
    if not lista:
        return "Non sono riuscito a dividere le domande. Controlla che nel file CSV ci siano 'a)', 'b)' e 'c)'."
    return lista
...
database = prepara_dati()

if isinstance(database, str):
    st.error(database)
else:
    if 'quiz_db' not in st.session_state:
        st.session_state.quiz_db = database
    ...
    if st.session_state.indice < len(st.session_state.quiz_db):
        ...
        if st.button("Conferma Risposta", use_container_width=True):
            if scelta:
                ...
                st.warning("Devi selezionare una risposta!")
    else:
        st.balloons()
