...
# --- DATABASE RISPOSTE (Le tue 318 risposte originali) ---
RISPOSTE = {
...
def prepara_dati():
    ...
    for m in trovate:
        ...
        lista.append({
            "id": id_q, "q": m.group(2).strip(), "a": m.group(3).strip(),
            "b": m.group(4).strip(), "c": m.group(5).strip(), "corretta": RISPOSTE.get(id_q, "A")
        })
    return lista
...
def inizia_esame():
    db_completo = prepara_dati()
    if isinstance(db_completo, list):
        st.session_state.quiz_db = random.sample(db_completo, min(30, len(db_completo)))
...
if not st.session_state.esame_attivo:
    ...
    st.write("### Regole dell'esame:")
    st.write("- ⏱ **Tempo:** 30 minuti")
    st.write("- 📝 **Domande:** 30 estratte casualmente")
    st.write("- ❌ **Limite Errori:** Massimo 4 (con 5 sei bocciato)")
    
    if st.button("INIZIA ESAME", use_container_width=True):
        inizia_esame()
        st.rerun()
    ...
    rimanente = max(0, 1800 - int(trascorso)) # 1800 secondi = 30 min
    
    mins, secs = divmod(rimanente, 60)
    
    # Barra superiore con statistiche
    col1, col2, col3 = st.columns(3)
    col1.metric("Domanda", f"{st.session_state.indice + 1}/30")
    col2.metric("Tempo", f"{mins:02d}:{secs:02d}")
    col3.metric("Errori", f"{st.session_state.errori}/4", delta_color="inverse")

    if rimanente <= 0:
        st.error("⏰ TEMPO SCADUTO!")
        st.session_state.finito = True

    if st.session_state.errori >= 5:
        st.error("🛑 ESAME FALLITO: Hai commesso 5 errori.")
        st.session_state.finito = True
    ...
    if not st.session_state.finito and st.session_state.indice < 30:
        ...
        if st.session_state.errori < 5 and rimanente > 0:
            st.balloons()
            st.success(f"🎉 ESAME SUPERATO! Punteggio: {st.session_state.punti}/30")
        else:
            st.error(f"❌ ESAME NON SUPERATO. Errori commessi: {st.session_state.errori}")
            
        if st.button("Riprova un altro Esame"):
            st.session_state.esame_attivo = False
            st.rerun()
