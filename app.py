...
st.set_page_config(page_title="Esame Tartufo", layout="centered")
...
with st.sidebar:
    ...
    if st.session_state.esame_attivo:
        if st.button("🔄 Ricomincia", type="primary", use_container_width=True):
            reset_quiz()
            st.rerun()

if not st.session_state.esame_attivo:
    st.title("🍄 Quiz Tartufo")
    st.write("Simulazione esame: 30 domande / 30 min / max 4 errori.")
    ...
    st.write(f"⏱ **{mins:02d}:{secs:02d}** | 📝 **{st.session_state.indice+1}/30** | ❌ **{st.session_state.errori}/4**")
    st.progress((st.session_state.indice) / 30)
    ...
    if not st.session_state.finito and st.session_state.indice < 30:
        ...
        scelta = st.radio("Risposta:", opzioni, index=None, key=f"r_{st.session_state.indice}", disabled=st.session_state.risposta_data)
        ...
        if not st.session_state.risposta_data:
            if st.button("CONFERMA", use_container_width=True):
                if scelta:
                    ...
                    if scelta[0] != item['corretta']: st.session_state.errori += 1
                    else: st.session_state.punti += 1
                    st.rerun()
        else:
            corretta = item['corretta']
            if st.session_state.ultima_scelta == corretta:
                st.success(f"Corretto! È la {corretta}")
            else:
                st.error(f"Sbagliato! Era la {corretta}")
        ...
        if st.session_state.errori < 5 and rimanente > 0:
            st.balloons()
            st.success(f"PROVA SUPERATA! {st.session_state.punti}/30")
        else:
            st.error(f"NON SUPERATO: {st.session_state.errori} errori.")
